...
`update.mjs` は3つに対応。**推奨は `espn`（APIキー不要・無料）**。

| プロバイダ | キー | 取得先 / 備考 | 既定の対象 |
|---|---|---|---|
| **`espn`（推奨）** | **不要** | ESPN の公開JSON。WC2026 をフル取得（順位表＋全日程）。非公式エンドポイントのため将来仕様変更の可能性あり | `fifa.world` / 2026 |
...
`espn` を使う場合、この手順（キー取得）は不要です。次の「2.」へ進んでください。
...
```dotenv
FOOTBALL_API_PROVIDER=espn   # 推奨。キー不要
FOOTBALL_API_KEY=            # espn では空でOK（football-data/api-football のときだけ記入）
```
...
2. **Settings → Secrets and variables → Actions**
   - **Variables** に `FOOTBALL_API_PROVIDER=espn`（未設定でも既定で `espn`）。
   - `espn` なら **Secret は不要**。`football-data` / `api-football` を使う場合のみ **Secrets** に `FOOTBALL_API_KEY` を追加。
